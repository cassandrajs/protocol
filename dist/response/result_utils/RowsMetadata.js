...
export class RowsMetadata {
    ...
    newResultMetadataId;
    flags;
    constructor(flags, columnSpecs, columnCount, pagingState, pkIndices, newResultMetadataId) {
    ...
    static fromColumnSpecs(columnSpecs, pagingState, pkIndices, newResultMetadataId) {
        return new RowsMetadata(RowsMetadata.computeFlags(false, columnSpecs, pagingState, newResultMetadataId), columnSpecs, columnSpecs.length, pagingState, pkIndices, newResultMetadataId);
    }
    static noMetadata(columnCount, pagingState, pkIndices, newResultMetadataId) {
        return new RowsMetadata(RowsMetadata.computeFlags(true, [], pagingState, newResultMetadataId), [], columnCount, pagingState, pkIndices, newResultMetadataId);
    }
    static computeFlags(noMetadata, columnSpecs, pagingState, newResultMetadataId) {
        let flags = 0;
        if (noMetadata) {
    ...
    encode(dest, encoder, withPkIndices, protocolVersion) {
        ...
        if (Flags.contains(this.flags, ProtocolConstants.RowsFlag.METADATA_CHANGED)) {
            encoder.writeShortBytes(this.newResultMetadataId, dest);
        }
        if (!Flags.contains(this.flags, ProtocolConstants.RowsFlag.NO_METADATA) &&
            this.columnSpecs.length > 0) {
            let globalTable = Flags.contains(this.flags, ProtocolConstants.RowsFlag.GLOBAL_TABLES_SPEC);
            if (globalTable) {
    ...
    encodedSize(withPkIndices, protocolVersion) {
    ...
    static decode(source, decoder, withPkIndices, protocolVersion) {
        ...
        const state = Flags.contains(flags, ProtocolConstants.RowsFlag.HAS_MORE_PAGES)
            ? decoder.readBytes(source)
            : null;
        const newResultMetadataId = Flags.contains(flags, ProtocolConstants.RowsFlag.METADATA_CHANGED)
        ...
        if (Flags.contains(flags, ProtocolConstants.RowsFlag.NO_METADATA)) {
            ...
            const globalTablesSpec = Flags.contains(flags, ProtocolConstants.RowsFlag.GLOBAL_TABLES_SPEC);
