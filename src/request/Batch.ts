...
export class Batch extends ProtocolMessage {
	...
	static Codec = class extends ProtocolMessageCodec {
		...
		encodedSize(message: ProtocolMessage): number {
			...
			if (!(queryCount <= 0xffff)) {
				throw new IllegalStateError(
					`Batch messages can contain at most ${0xffff} queries`
				)
			}

			if (!(batch.values.length === queryCount)) {
				throw new IllegalStateError(
					`Batch contains ${queryCount} queries but ${batch.values.length} value lists`
				)
			}
		...
		decode<B>(source: B, decoder: PrimitiveCodec<B>): ProtocolMessage {
			...
			const queriesOrIds: (string | number[])[] = []
			const values: Array<Array<Uint8Array>> = []
			for (let i = 0; i < queryCount; i++) {
			...
			const keyspace =
				(flags & (1 << 2)) !== 0 ? decoder.readString(source) : null
			const nowInSeconds =
				(flags & (1 << 3)) !== 0
					? decoder.readInt(source)
